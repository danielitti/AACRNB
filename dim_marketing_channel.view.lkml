...
FROM  (SELECT * FROM SHARED_MRT_UAT7.DIM_SOURCE_CODE) SC
  LEFT JOIN
      (SELECT * FROM SHARED_MRT_UAT7.DIM_MARKETING_CHANNEL WHERE MARKETING_CHANNEL_LEVEL  IN ('0', '2')) LVL2
   ON SC.MARKETING_CHANNEL_KEY = LVL2.MARKETING_CHANNEL_KEY
    ...
    FROM SHARED_MRT_UAT7.DIM_MARKETING_CHANNEL WHERE MARKETING_CHANNEL_LEVEL IN ('1' ,'0')) LVL1

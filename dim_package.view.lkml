view: package {
  derived_table: {
    sql: SELECT LVL2.PACKAGE_KEY AS PACKAGE_LEVEL_2_KEY,
        ...
        FROM  (SELECT * FROM SHARED_MRT_UAT7.DIM_PACKAGE WHERE PACKAGE_LEVEL  IN ('0', '2')) LVL2
        LEFT JOIN
              (SELECT PARENT_PACKAGE_KEY, PACKAGE_KEY, PACKAGE_DESC AS LEVEL_1_PACKAGE_DESC , PACKAGE_FULL_DESC AS LEVEL_1_PACKAGE_FULL_DESC FROM SHARED_MRT_UAT7.DIM_PACKAGE WHERE PACKAGE_LEVEL IN ('1' ,'0')) LVL1
